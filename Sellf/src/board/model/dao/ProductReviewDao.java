...
public class ProductReviewDao {
	...
	public ArrayList<ProductReview> getCurrentPage(Connection conn, int currentPage, int recordCountPerPage) {
		...
		String query = "SELECT * FROM (select user_review_tb.*, row_number() over(order by user_rv_pk desc) as num from user_review_tb)\" \r\n" + 
		...
		try {
			...
			while(rset.next())
			{
				...
				r.setUser_rv_pk(rset.getInt("user_rv_pk"));
				r.setUser_rv_user_entire_id_fk(rset.getString("user_rv_user_entire_id_fk"));
				r.setUser_rv_user_entire_rved_id_fk(rset.getString("user_rv_user_entire_rved_id_fk"));
				r.setUser_rv_product_entire_fk(rset.getInt("user_rv_product_entire_fk"));
				r.setUser_review_comment(rset.getString("user_review_comment"));
				r.setUser_review_rating(rset.getInt("user_review_rating"));
				r.setUser_review_date(rset.getDate("user_review_date"));
				list.add(r);
			}
	...
	public String getPageNavi(Connection conn, int currentPage, int recordCountPerPage, int naviCountPerPage) {
		...
		String query = "select count(*) as totalCount from user_review_tb";
		...
		//페이지 navi에서 사용할 '<' 모양과 '>'모양을 사용하기 위해 
				...
				if(needPrev) { //시작이 1페이지가 아니라면! 
					sb.append("<a href='/review?currentPage="+(startNavi-1)+"')> < </a>");// 이전으로 
					sb.append("<a href='/review?currentPage="+1+"')> << </a>"); //맨앞으로 
				}
				...
				if(needNext)
				{
					sb.append("<a href='/review?currentPage="+(endNavi+1)+"'> > </a>"); // 다음으로
					sb.append("<a href='/review?currentPage="+(pageTotalCount)+"'> >> </a>");//맨뒤로 
				}
				return sb.toString();
	}
